...
namespace EA::cli {
    ...
    turbo::Status make_zone_create(EA::discovery::DiscoveryManagerRequest *req) {
        EA::discovery::ZoneInfo *zone_req = req->mutable_zone_info();
        req->set_op_type(EA::discovery::OP_CREATE_ZONE);
    ...
    turbo::Status make_zone_remove(EA::discovery::DiscoveryManagerRequest *req) {
        EA::discovery::ZoneInfo *zone_req = req->mutable_zone_info();
        req->set_op_type(EA::discovery::OP_DROP_ZONE);
        auto rs = check_valid_name_type(ZoneOptionContext::get_instance()->namespace_name);
        if (!rs.ok()) {
            return rs;
        }
    ...
    turbo::Status make_zone_modify(EA::discovery::DiscoveryManagerRequest *req) {
        req->set_op_type(EA::discovery::OP_MODIFY_ZONE);
        EA::discovery::ZoneInfo *zone_req = req->mutable_zone_info();
        auto rs = check_valid_name_type(ZoneOptionContext::get_instance()->namespace_name);
        if (!rs.ok()) {
            return rs;
