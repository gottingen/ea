...
namespace EA::servlet {
    void NamespaceManager::create_namespace(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        ...
        if (_namespace_id_map.find(namespace_name) != _namespace_id_map.end()) {
            ...
            IF_DONE_SET_RESPONSE(done, EA::servlet::INPUT_PARAM_ERROR, "namespace already existed");
            return;
        }
    ...
    void NamespaceManager::drop_namespace(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
    ...
    void NamespaceManager::modify_namespace(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        auto &namespace_info = request.namespace_info();
        std::string namespace_name = namespace_info.namespace_name();
        if (_namespace_id_map.find(namespace_name) == _namespace_id_map.end()) {
            TLOG_WARN("request namespace:{} not exist", namespace_name);
        ...
        //目前支持改quota
        int64_t namespace_id = _namespace_id_map[namespace_name];
        EA::servlet::NameSpaceInfo tmp_info = _namespace_info_map[namespace_id];
    ...
    int NamespaceManager::load_namespace_snapshot(const std::string &value) {
        EA::servlet::NameSpaceInfo namespace_pb;
