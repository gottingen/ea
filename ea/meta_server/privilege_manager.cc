...
namespace EA::servlet {
    ...
    void PrivilegeManager::create_user(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        auto &user_privilege = const_cast<EA::servlet::UserPrivilege &>(request.user_privilege());
        ...
        if (_user_privilege.find(username) != _user_privilege.end()) {
            ...
            IF_DONE_SET_RESPONSE(done, EA::servlet::INPUT_PARAM_ERROR, "username has been repeated");
            return;
        }
    ...
    void PrivilegeManager::drop_user(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        std::string username = request.user_privilege().username();
        if (_user_privilege.find(username) == _user_privilege.end()) {
            TLOG_WARN("request username not exist, username:{}", username);
            IF_DONE_SET_RESPONSE(done, EA::servlet::INPUT_PARAM_ERROR, "username not exist");
            return;
        }
    ...
    void PrivilegeManager::add_privilege(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        auto &user_privilege = const_cast<EA::servlet::UserPrivilege &>(request.user_privilege());
        std::string username = user_privilege.username();
        if (_user_privilege.find(username) == _user_privilege.end()) {
            TLOG_WARN("request username not exist, username:{}", username);
        ...
        EA::servlet::UserPrivilege tmp_mem_privilege = _user_privilege[username];
    ...
    void PrivilegeManager::drop_privilege(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        auto &user_privilege = const_cast<EA::servlet::UserPrivilege &>(request.user_privilege());
        std::string username = user_privilege.username();
        if (_user_privilege.find(username) == _user_privilege.end()) {
            TLOG_WARN("request username not exist, username:{}", username);
    ...
    int PrivilegeManager::load_snapshot() {
        ...
        for (; iter->Valid(); iter->Next()) {
            ...
            EA::servlet::UserPrivilege user_privilege;
    ...
    void PrivilegeManager::insert_zone_privilege(const EA::servlet::PrivilegeZone &privilege_zone,
        ...
        if (!whether_exist) {
            EA::servlet::PrivilegeZone *ptr_zone = mem_privilege.add_privilege_zone();
            *ptr_zone = privilege_zone;
        }
    }

    void PrivilegeManager::insert_servlet_privilege(const EA::servlet::PrivilegeServlet &privilege_servlet,
                                                    EA::servlet::UserPrivilege &mem_privilege) {
        bool whether_exist = false;
        ...
        if (!whether_exist) {
            EA::servlet::PrivilegeServlet *ptr_table = mem_privilege.add_privilege_servlet();
    ...
    void PrivilegeManager::delete_zone_privilege(const EA::servlet::PrivilegeZone &privilege_zone,
                                                 EA::servlet::UserPrivilege &mem_privilege) {
        EA::servlet::UserPrivilege copy_mem_privilege = mem_privilege;
    ...
    void PrivilegeManager::delete_servlet_privilege(const EA::servlet::PrivilegeServlet &privilege_servlet,
                                                    EA::servlet::UserPrivilege &mem_privilege) {
        int64_t zone_id = privilege_servlet.zone_id();
        int64_t servlet_id = privilege_servlet.servlet_id();
        EA::servlet::UserPrivilege copy_mem_privilege = mem_privilege;
