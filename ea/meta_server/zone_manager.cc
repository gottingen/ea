...
namespace EA::servlet {
    void ZoneManager::create_zone(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        ...
        if (_zone_id_map.find(zone_name) != _zone_id_map.end()) {
            ...
            IF_DONE_SET_RESPONSE(done, EA::servlet::INPUT_PARAM_ERROR, "zone already exist");
            return;
        }
    ...
    void ZoneManager::drop_zone(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        // check
        ...
        if (!_servlet_ids[zone_id].empty()) {
            ...
            IF_DONE_SET_RESPONSE(done, EA::servlet::INPUT_PARAM_ERROR, "zone has servlet");
            return;
        }
        // persist to rocksdb
    ...
    void ZoneManager::modify_zone(const EA::servlet::MetaManagerRequest &request, braft::Closure *done) {
        auto &zone_info = request.zone_info();
        std::string namespace_name = zone_info.namespace_name();
        std::string zone_name = namespace_name + "\001" + zone_info.zone();
        int64_t namespace_id = NamespaceManager::get_instance()->get_namespace_id(namespace_name);
        ...
        EA::servlet::ZoneInfo tmp_zone_info = _zone_info_map[zone_id];
