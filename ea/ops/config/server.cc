...
int main(int argc, char **argv) {
    google::SetCommandLineOption("flagfile", "conf/eaconfig_gflags.conf");
    ...
    auto dir = turbo::filesystem::path(EA::FLAGS_config_db_path).parent_path();
    std::error_code ec;
    if(!turbo::filesystem::exists(dir, ec)) {
        turbo::filesystem::create_directories(dir);
    }

    auto rs = EA::rdb::Storage::get_instance()->init(EA::FLAGS_config_db_path);
    if (rs < 0) {
    ...
    if (0 != braft::add_service(&server, EA::FLAGS_config_listen.c_str())) {
    ...
    bool use_bns = false;
    //指定的是ip:port的形式
    std::vector<std::string> list_raft_peers = turbo::StrSplit(EA::FLAGS_config_server_bns, ',');
    ...
    EA::config::ConfigServer *service_server = EA::config::ConfigServer::get_instance();
    ...
    //启动端口
    if (server.Start(EA::FLAGS_config_listen.c_str(), nullptr) != 0) {
    ...
    TLOG_INFO("config service server init success");
