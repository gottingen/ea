...
#ifndef EA_RPC_DICT_SERVER_INTERACT_H_
#define EA_RPC_DICT_SERVER_INTERACT_H_
...
namespace EA::rpc {
    ...
    class DictServerInteract {
    public:
        static const int RETRY_TIMES = 5;

        static DictServerInteract *get_instance() {
            static DictServerInteract _instance;
            return &_instance;
        }

        DictServerInteract() = default;
        ...
        int send_request(const std::string &service_name,
                         ...
                         Response &response) {
            const ::google::protobuf::ServiceDescriptor *service_desc = proto::DictService::descriptor();
            ...
            do {
                if (retry_time > 0 && FLAGS_dict_time_between_connect_error_ms > 0) {
                    bthread_usleep(1000 * FLAGS_dict_time_between_connect_error_ms);
                }
                ...
                if (leader_address.ip != butil::IP_ANY) {
                    ...
                    if (short_channel.Init(leader_address, &channel_opt) != 0) {
                        TLOG_WARN("connect with dict server fail. channel Init fail, leader_addr:{}",
                    ...
                    if (!cntl.Failed() && response.errcode() == proto::SUCCESS) {
                        ...
                        TLOG_INFO("connect with dict server success by bns name, leader:{}",
                                  butil::endpoint2str(cntl.remote_side()).c_str());
                        return 0;
                    }
                }

                TLOG_TRACE("dict_req[{}], dict_resp[{}]", request.ShortDebugString(), response.ShortDebugString());
                if (cntl.Failed()) {
                ...
                if (response.errcode() == proto::NOT_LEADER) {
                    TLOG_WARN("connect with dict server:{} fail. not leader, redirect to :{}, log_id:{}",
                              butil::endpoint2str(cntl.remote_side()).c_str(),
                ...
                if (response.errcode() != proto::SUCCESS) {
                    TLOG_WARN("send dict server fail, log_id:{}, response:{}", cntl.log_id(),
...
#endif  // EA_RPC_DICT_SERVER_INTERACT_H_
