...
namespace EA::rpc {
    ...
    class PluginServerInteract {
    public:
        static const int RETRY_TIMES = 5;

        static PluginServerInteract *get_instance() {
            static PluginServerInteract _instance;
            return &_instance;
        }

        PluginServerInteract()  = default;
        ...
        int send_request(const std::string &service_name,
                         ...
                         Response &response) {
            const ::google::protobuf::ServiceDescriptor *service_desc = proto::PluginService::descriptor();
            ...
            do {
                if (retry_time > 0 && FLAGS_plugin_time_between_connect_error_ms > 0) {
                    bthread_usleep(1000 * FLAGS_plugin_time_between_connect_error_ms);
                }
                ...
                if (leader_address.ip != butil::IP_ANY) {
                    ...
                    if (short_channel.Init(leader_address, &channel_opt) != 0) {
                        TLOG_WARN("connect with plugin server fail. channel Init fail, leader_addr:{}",
                    ...
                    if (!cntl.Failed() && response.errcode() == proto::SUCCESS) {
                        ...
                        TLOG_INFO("connect with plugin server success by bns name, leader:{}",
                                  butil::endpoint2str(cntl.remote_side()).c_str());
                        return 0;
                    }
                }

                TLOG_TRACE("plugin_req[{}], plugin_resp[{}]", request.ShortDebugString(), response.ShortDebugString());
                if (cntl.Failed()) {
                ...
                if (response.errcode() == proto::NOT_LEADER) {
                    TLOG_WARN("connect with plugin server:{} fail. not leader, redirect to :{}, log_id:{}",
                              butil::endpoint2str(cntl.remote_side()).c_str(),
                ...
                if (response.errcode() != proto::SUCCESS) {
                    TLOG_WARN("send plugin server fail, log_id:{}, response:{}", cntl.log_id(),
